...
def build_model(embedding_matrix):
    # modified from github.com/alexander-rakhlin/CNN-for-Sentence-Classification-in-Keras/blob/master/sentiment_cnn.py
    dropout1 = Dropout(0.15)
    maxPooling1D = MaxPooling1D(pool_size=2)
    flatten = Flatten()
    convs = []
    for sz in (1, 3, 5, 7, 9, 11, 13):  # filter_sizes
        conv1D = Convolution1D(filters=1000,
                               kernel_size=sz,
                               padding="valid",
                               activation="relu",
                               strides=1)
    ...
    merged = Dense(250, activation=ACT)(merged)
    merged = Dropout(0.15)(merged)
    merged = BatchNormalization()(merged)
