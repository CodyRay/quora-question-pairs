...
from keras.layers import Dense, Input, Convolution1D, Embedding, Dropout, MaxPooling1D, Flatten, Concatenate
...
def build_model(nb_words, embedding_matrix):
    ...
    # Code modified from https://github.com/alexander-rakhlin/CNN-for-Sentence-Classification-in-Keras/blob/master/sentiment_cnn.py
    dropout1 = Dropout(0.15)
    maxPooling1D = MaxPooling1D(pool_size=2)
    flatten = Flatten()
    convs = []
    for sz in (1, 3, 5, 7, 9, 11, 13):  # filter_sizes
        conv1D = Convolution1D(filters=100,
                               kernel_size=sz,
                               padding="valid",
                               activation="relu",
                               strides=1)
    ...
    def make_layers_for_input(input):
        z = dropout1(input)
    ...
    sequence_1_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
    embedded_sequences_1 = embedding_layer(sequence_1_input)
    x1 = make_layers_for_input(embedded_sequences_1)

    sequence_2_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
    embedded_sequences_2 = embedding_layer(sequence_2_input)
    y1 = make_layers_for_input(embedded_sequences_2)
...
def main():
    ...
    model = build_model(nb_words, embedding_matrix)
